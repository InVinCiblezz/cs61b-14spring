...
class SibTreeNode extends TreeNode {
  ...
  public TreeNode parent() throws InvalidNodeException {
    ...
    if (this.equals(myTree.root)) return new SibTreeNode();
    return this.parent;
  }
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if (isValidNode()) {
      ...
      node.parent = this;
      SibTreeNode firstSibling = firstChild;

      if (c <= 1) {
        node.nextSibling = firstSibling;
        firstChild = node;
      } else {
        SibTreeNode prevNode = firstSibling;
        SibTreeNode kid = firstSibling;
        while ((kid != null) && (c > 1)) {
          prevNode = kid;
          kid = kid.nextSibling;
          c--;
  ...
  public void removeLeaf() throws InvalidNodeException {
    ...
    SibTreeNode root = myTree.root;
    //remove root
    if (this.equals(root)) {
      myTree.root = new SibTreeNode();
      return;
    }
    //remove leaf
    ...
    //remove firstChild
    if (this.equals(kid)) {
      parentNode.firstChild = kid.nextSibling;
    } else {
      SibTreeNode prevNode = kid;
      while (kid != null) {
        if (this.equals(kid)) {
