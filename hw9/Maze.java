...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    int vTotal = vert * (horiz - 1);
    int hTotal = horiz * (vert - 1);
    int[] walls = new int[vTotal + hTotal];

    //random
    for(i = 0; i < hTotal + vTotal; i++) walls[i] = i;
    for(i = vTotal + hTotal - 1; i > 0; i--){
      int index = randInt(i);
      int temp = walls[index];
      walls[index] = walls[i];
      walls[i] = temp;
    }
    ...
    for(i = 0; i < walls.length; i++){
      if(walls[i] < hTotal){
        // up bottom in horizontal
        x = (walls[i]) % horiz;
        y = (walls[i]) / horiz;
        int upperCell = walls[i];
        int bottomCell = upperCell + horiz;
        ...
        // left right in vertical
        x = (walls[i] - hTotal) % (horiz - 1);
        y = (walls[i] - hTotal) / (horiz - 1);
